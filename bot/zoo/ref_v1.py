...
# Ver Agent.act(). Evita neutralizar celdas por colision con un rival que calcule
# la misma ruta, y es lo que hace viable el self-play como sparring.
BREAK_SYMMETRY = True
        ...
        while pq:
            ...
            for v in b.nb[u]:
                ...
                if blocked_owner is not None and st.track[v] == blocked_owner:
                    continue
                w = 0 if st.passable(v) else b.cost[v]
                nd = d + w
                if nd < dist[v]:
                    dist[v] = nd
                    par[v] = u
...
class Agent:
    ...
    def act(self, st):
        ...
        # Romper la simetria: el jugador 1 recorre el plan desde el otro extremo.
        # Dos bots que calculan la misma ruta mas barata colocan si no en las
        # MISMAS celdas el mismo turno, y esas celdas quedan neutrales (owner 2),
        # que no puntuan a nadie. Construyendo desde extremos opuestos cada uno
        # se queda con su mitad. Ademas hace posible el self-play: en espejo
        # perfecto todas las partidas acababan 0-0.
        if BREAK_SYMMETRY and st.board.my_id == 1:
            cells = cells[::-1]
        placed = set()
        for i in cells:
            if paint <= 0:
